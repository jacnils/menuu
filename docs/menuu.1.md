...
**Menuu** is an **X11** and **Wayland** menu application which takes standard input, parses
...
`menuu_run`, it can serve as a run launcher.

## Usage

On runtime, Menuu reads from standard input (stdin). Menuu items are
separated by a newline (`\n`). When (by default) Enter is pressed, the selected
item will be piped to stdout.

This allows things like `printf "Apple\nOrange\nPear\n" | menuu`. This command
will spawn an Menuu window with three items, 'Apple', 'Orange' and 'Pear'. The
...
Note that Menuu doesn't strictly read standard input, it can read from files
too using the `-lf` or `--list-file` argument.

On top of this, you can specify arguments to change the behavior of Menuu.
See a list below for a list.

## Arguments

You may use long, descriptive arguments or the shorter arguments. Note that none
need to be specified for a basic Menuu prompt. The most commonly used arguments
...
:   Set Menuu line height to height
...
`-p, --prompt text`
:   Set Menuu prompt text to text

`-pt, --pretext text`
:   Set Menuu pretext to text
...
:   Offset Menuu x position by x offset (X11 only)

`-y, --y-position y offset`
:   Offset Menuu y position by y offset (X11 only)

`-n, --preselect line`
:   Preselect line line in the list of items

`-z, --width width`
:   Width of the Menuu window
...
`-nm, --normal`
:   Start Menuu in normal mode

`-im, --insert`
:   Start Menuu in insert mode

`-t, --top`
:   Position Menuu at the top of the screen

`-b, --bottom`
:   Position Menuu at the bottom of the screen

`-c, --center`
:   Position Menuu at the center of the screen
...
`-m, --monitor monitor`
:   Specify a monitor to run Menuu on (X11 only)

`-w, --embed window id`
:   Embed Menuu inside window id (X11 only)
...
`-ir, --image-resize`
:   Allow Menuu to resize itself to fit the image

`-nir, --no-image-resize`
:   Don't allow Menuu to resize itself to fit the image
...
:   Spawn Menuu as a window manager controlled client/window (X11 only)

`-nwm, --unmanaged`
:   Don't spawn Menuu as a window manager controlled client/window (X11 only)
...
`-lcfg, --load-config`
:   Load menuu.configuration (~/.config/menuu/menuu.conf) on runtime

`-ncfg, --no-load-config`
:   Don't load menuu.configuration (~/.config/menuu/menuu.conf) on runtime

`-x11, --x11`
:   Run Menuu in X11 mode

`-wl, --wayland`
:   Run Menuu in Wayland mode

`-v, --version`
:   Print Menuu version to stdout

`-rv, --raw-version`
:   Print raw Menuu version number to stdout

`-fl, --feature-list`
:   List the state of all features that can be toggled

`-fn, --font font`
:   Set the Menuu font to font
...
Menuu also supports regex matching, but it is not enabled by default. Therefore,
...
Normal mode is the mode Menuu starts in unless a mode argument is specified
or another mode is set in the configuration file. Note that if `force_insert_mode`
is enabled, Normal mode cannot be used and Menuu will start in Insert mode
instead.
...
exit Menuu, allowing you to fix your Menuu configuration.

## History buffer

Menuu allows you to specify a history file using the `-H` argument. When this
argument is specified, the selected item(s) will be appended to the file.
In menuu.conf, you can specify a max number of entries, and whether you
...
Menuu has a `-p` or `--prompt` option. It allows you to specify
text to display next to the item list. It is displayed on the left side of the
Menuu window. It should be noted that the prompt is purely visual though.
...
Menuu supports displaying images. This image is placed on the left side of
the menu window, as long as Menuu isn't a single line.

To use an image, pipe `img:///path/to/image` to menuu. If you want you can
...
Menuu supports colored text through SGR sequences. This is the same colors
that you might already be using in your shell scripts. This means you can
pipe practically any colored shell script straight into Menuu,
...
SGR sequences (ANSI escape codes) can be used to set the color of Menuu items. Here's
...
are built into Menuu and cannot be overridden. They are only really implemented
into Menuu for compatibility, in practice you should use true color sequences
...
Menuu exposes its functions to external programs through a socket. You can use e.g.
`socat` to interact with it. The default file descriptor is `/tmp/menuu.sock`, and
...
`echo '{"calls": [{"function": "toggle_prompt"}]}' | socat - UNIX-CONNECT:/tmp/menuu.sock`
...
with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client_socket:
    client_socket.connect("/tmp/menuu.sock")
...
The built-in libmenuu uses this socket. If you can use Python, it is strongly
recommended that you use the API instead.

## Pango markup and text formatting

Because Menuu uses Pango and Cairo for text drawing, Pango markup can be used
...
to color text in Menuu because SGR sequences can be used instead.
...
`... | menuu ... | sed 's/<big>//g; s/</big>//g'`

See [this page](https://docs.gtk.org/Pango/pango_markup.html) for more information.

## Configuration

Unlike dmenu, Menuu has a configuration file which can be edited by hand. It
is located in ~/.config/menuu/menuu.py, but you can override this by
...
after installing Menuu. This configuration file has identical keybinds to the
...
you cannot exit Menuu, press `Ctrl+Alt+Delete`. This is a hardcoded keybind
...
Menuu supports mouse clicks. The following parts can be clicked on:

- `ClickWindow`
  - Clicking on the Menuu window
- `ClickPrompt`
...
Menuu also has .Xresources (xrdb) support built in. It reads the xrdb
...
`menuu.color0` through `menuu.color15` and `menuu.font` preferences
are read by Menuu, these alter the font and colorscheme Menuu uses.

| Xresources name    | menuu name   |
| :----------------- | :-------------|
| font               | font          |
| color10            | caretfg       |
| color0             | caretbg       |
...
- `screenshot`
  - Take a screenshot of Menuu

## License

Menuu is licensed under the MIT license. The documentation is licensed
under the CC-BY-SA 4.0 license.

## See also

- [Menuu Git repository](https://github.com/jacnils/menuu)
- [menuu_run](https://github.com/jacnils/menuu_run)
- [menuuify](https://github.com/jacnils/menuuify)
- [menuu-themes](https://github.com/jacnils/menuu-themes)
